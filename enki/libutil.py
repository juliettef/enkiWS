...
def urlencode( fields ):
	...
	# ordered (list of tuples)
		...
		for field in fields:
			...
			if isinstance( item, unicode):
				item = item.encode('utf8')
			fields_non_unicode.append(( field[ 0 ], item ))
...
def markdown_escaped_extras( text ):
	...
	return markdown2.markdown( text, safe_mode = 'escape', extras = { 'nofollow' : {}, 'fenced-code-blocks' : { 'linenos': True }, 'html-classes' : { 'img' : 'img-responsive' }})
